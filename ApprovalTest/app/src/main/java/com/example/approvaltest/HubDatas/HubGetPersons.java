...
public class HubGetPersons extends AsyncTask<Void, Void, ArrayList<ResponsibleSubject> >{
	// Initialize
	Models.Settings mysettings;
	JSONArray myarray;
	Context myctx;
	public ArrayList<ResponsibleSubject> mylist = new ArrayList<ResponsibleSubject>();
	private static ObjectMapper mymapper = new ObjectMapper();
	
	

	public HubGetPersons(Context ctx) {
		myctx = ctx;
	...
	@SuppressLint("NewApi")
	@Override
	protected ArrayList<ResponsibleSubject> doInBackground(Void... params) {
		mysettings = (Settings) new DataHandler().getObjectFromFile(
				DataHandler.SETTINGS_FILEMANE, myctx);
				
		String collection = "ResponsibleSubjects";
		String expand = null;
		String filter = null;
		...
		{		
			...
			ResponsibleSubject object = mymapper.readValue(myobj.toString(), ResponsibleSubject.class);
			mylist.add(object);
