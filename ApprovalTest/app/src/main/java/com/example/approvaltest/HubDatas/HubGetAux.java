...
public class HubGetAux extends AsyncTask<Void, Void, ArrayList<AuxillaryCondition> >{
	// Initialize
	Models.Settings mysettings;
	JSONArray myarray;
	Context myctx;
	public ArrayList<AuxillaryCondition> mylist = new ArrayList<AuxillaryCondition>();
	private static ObjectMapper mymapper = new ObjectMapper();
	
	

	public HubGetAux(Context ctx) {
		myctx = ctx;
	...
	@SuppressLint("NewApi")
	@Override
	protected ArrayList<AuxillaryCondition> doInBackground(Void... params) {
		mysettings = (Settings) new DataHandler().getObjectFromFile(
				DataHandler.SETTINGS_FILEMANE, myctx);
				
		String collection = "AuxillaryConditions";
		String expand = "ConditionInspections,ConditionInspections/EntryDate,ConditionInspections/Measures";
		String filter = null;
		...
		{		
			...
			AuxillaryCondition object = mymapper.readValue(myobj.toString(), AuxillaryCondition.class);
			mylist.add(object);
